...
try {
  ...
  console.trace("my error", error, customElements);
}
...
const Polymer = function (info) {
  ...
  try {
    ...
    console.info("error defining", klass.is);
  }
...
document.head.appendChild(style); /// BareSpecifier=@polymer\iron-overlay-behavior\iron-focusables-helper
...
const IronResizableBehavior = {
   ...
   */notifyResize: function () {
    ...
    if (/firefox/i.test(navigator.userAgent) && window.fromToolbar) {
      // patch: let's skip resize
    } else {
      this._fireResize();
    }
  },
...
document.head.appendChild(template$1.content); /// BareSpecifier=@polymer\paper-styles\default-theme
...
document.head.appendChild(template$2.content); /// BareSpecifier=@polymer\paper-styles\shadow
...
document.head.appendChild(template$3.content); /// BareSpecifier=@polymer\paper-styles\typography
