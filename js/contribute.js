...
async function initCurrencyAndMinimums(currencyCode) {
	if (licenseType == "multipleUsers") {
		...
		if (currencyCode == "BTC") {
			if (await isEligibleForReducedDonation(storage)) {
...
function showSuccessfulPayment() {
	...
	if (DetectClient.isFirefox()) {
		...
		$("#video").attr("src", "https://www.youtube.com/embed/Ue-li7gl3LM?rel=0&autoplay=1&theme=light");
...
function signOut() {
	location.href = "https://accounts.google.com/Logout?continue=https://drive.google.com";
}
...
async function sendStats(paymentProcessor) {
	...
	sendGA(paymentProcessor, "success", "daysElapsedSinceFirstInstalled", await daysElapsedSinceFirstInstalled(storage));
...
$(document).ready(() => {
	...
	if (DetectClient.isFirefox()) {
		...
		$("#video").attr("src", "https://www.youtube.com/embed/pN9aec4QjRQ?theme=light&enablejsapi=1");
	}

	getStorage().then(thisStorage => {
		storage = thisStorage;
		email = getEmailFromStorage(storage);
		ensureEmail(true);
        ...
        */
		...
		var randomNumber = Math.floor(Math.random() * 2) + 1;
			if (randomNumber == 1) { // Math.random() < 0.5
			localStorage._minOnetimePayscale = "10-20-40-60";
			} else if (randomNumber == 2) {
			localStorage._minOnetimePayscale = "10-15-20-40";
			$("#onetimeAmountSelections [amount='60']").attr("hidden", true);
			$("#onetimeAmountSelections [amount='15']").unhide();
			}
		*/
		}
	...
	$("#changeDomain").click(function() {
		niceAlert(getMessage("changeThisDomain", getMessage("changeThisDomain2"))).then(function(response) {
			if (response == "other") {
				//signOut();
			}
		});
